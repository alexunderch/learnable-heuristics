...
class SaILAgent():
  ...
  def run_training(self, train_folder, train_oracle_folder, validation_folder, validation_oracle_folder, model_folder, file_start_num_train, file_start_num_valid, pretrained_model, visualize_train, visualize_validation, oracle_file_type="json"):
    ...
    for i in range(self.N):
      ...
      avg_loss_train = self.heuristic_fn.train(agg_dataset) #Regression loss in this iteration of training #! heuristic func
  ...
  def run_validation(self, validation_folder, validation_oracle_folder, file_start_num_valid, visualize_validation, oracle_file_type="json"):
    ...
    avg_loss = 0
    num_unsovable = 0
    for j in range(self.maps_val_num):
    ...
    legit_envs = self.maps_val_num - num_unsovable
    avg_expansions /= self.maps_val_num
    avg_path_cost /= self.maps_val_num
    avg_time_taken /= self.maps_val_num
    avg_loss /= self.maps_val_num
